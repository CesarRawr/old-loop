...
export const getDataToSend = (
  ...
  const dataToSend: ModifyData[] = selectedDevices.map((newDevice: Item) => {
    ...
    const originalDevice: MetaDispositivo | undefined = selectedLoan?.dispositivos.filter((loanDevice: MetaDispositivo) => {
...
export const getDeletedDevices = (
  ...
  const deletedDevices: (ModifyData | undefined)[] | undefined = selectedLoan?.dispositivos.map((loanDevice: MetaDispositivo) => {
    const isThere: boolean = !!selectedDevices.filter((newDevice: Item) => newDevice._id === loanDevice._id)[0];
