...
export const getTodayCourses = (nrcs: NrcTag[], selectedItem: any): NrcTag[] => {
  return nrcs.filter((nrc: NrcTag) => nrc.materia._id === selectedItem._id);
}

// Obtener los horarios de los cursos de hoy
export const getTodayCoursesSchedule = (todayCourses: NrcTag[]): (NrcTag | undefined)[] => {
  const dayName = getDayName();
  return todayCourses.map((course: NrcTag) => {
...
export const getOrderedSchedules = (schedules: (NrcTag | undefined)[]): (NrcTag | undefined)[] => {
  return !schedules ? []: schedules.sort((a: any, b: any) => {
...
export const setLoanData = (
  ...
  // Setteando maestro
  setValue('maestros', nearestCourse.maestro ? {
