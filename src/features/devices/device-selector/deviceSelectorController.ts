...
export const selectOption = (selectedItems: any, value: any): any => {
  const selected: Item[] = selectedItems.map((blockedItem: Item, index: number, array: Item[]) => { 
  ...
  const [result]: Item[] = value.filter((item: Item) => item.value === lastItem.value);
  // Si existe el tag, modifica la cantidad, si no, agrega el nuevo item
  const tags: Item[] = !!result ? (
    value.map((item: Item) => {
      return (item.value !== lastItem.value) ? item: {
        ...item,
        value: item.value,
